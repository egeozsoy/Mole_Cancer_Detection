...
if __name__ == '__main__':
    ...
    weights = {
        ...
        'wc1': tf.Variable(tf.random_normal([conv_size, conv_size, 1, output_1])),  # image res 80*80
        'wc2': tf.Variable(tf.random_normal([conv_size, conv_size, output_1, output_2])),  # image res 40*40
        'wd1': tf.Variable(tf.random_normal([img_size // 4 * img_size // 4 * output_2, output_3])),  # image res 20*20
    ...
    testing_images = testing_images.reshape(-1, 6400)

    model_name = 'model_{}_{}_{}_{}_{}_{}_{}'.format(learning_rate, output_1, output_2, output_3, conv_size, dropout,
                                                     img_size
                                                     )
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            loss, acc, summary = sess.run([loss_op, accuracy, merged_summary_op],
                                          feed_dict={X: testing_images, Y: testing_labels, keep_prob: 1.0})
            summary_writer.add_summary(summary, epoch)
            print("Completed {}".format(epoch),
                  "Minibatch Loss= " + "{:.1f}".format(loss) + ", Training Accuracy= " + "{:.3f}".format(acc))
